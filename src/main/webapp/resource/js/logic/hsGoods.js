$(function() {
	...
	$("#head_hotsale").addClass('active_leftNav');
});
...
//        var effectiveDateStr= $("#effectiveDateStr-serch-key").val();
        //资源单页面查询资源单
        $.ajax({
            url:getRootPath()+"/logic/hsGoodsPageForClient",
            type:"get",
...
//	        html.push("<td > "+moment(data[i].effectiveDate).format("YYYY-MM-DD")+" </td>");
	        //操作
	        html.push("<td > " +
        			" <a  class='btn-gc cell' onclick='hsGoodsPage.showDescription(\""+data[i].description+"\")' ><i class='glyphicon glyphicon-paperclip'></i>&nbsp;特卖说明</a>" +
        			"</td>");
            html.push("</tr>");
        }
